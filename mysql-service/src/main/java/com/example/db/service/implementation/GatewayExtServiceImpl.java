...
public class GatewayExtServiceImpl implements GatewayExtService {
    ...
    @Override
    public RateResponseDto getLatestRates(GetawayRequestDTO request) {
        statisticsService.verifyRequest(request.getRequestId());
        statisticsService.createRecord(request.getServiceName(), request.getRequestId(), request.getClient());
        return convertToDto(rateService.getLatestRateForCurrency(request.getCurrency()));
    }

    @Override
    public Set<RateResponseDto> getHistoryRates(GetawayRequestDTO request) {
        statisticsService.verifyRequest(request.getRequestId());
        statisticsService.createRecord(request.getServiceName(), request.getRequestId(), request.getClient());
